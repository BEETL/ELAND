...
sub _printSupportTxt($$)
{
    ...
    my $data = Data::Dumper->new([{
        ...
        'PERL_VERSION' => "$^V", 
        'PERL_EXECUTABLE' => "$^X",
    }, '@CASAVA_VERSION_FULL@', $0, $commandLine, \@INC, [sort(values %INC)], \%ENV],
...
sub initialize
{
    ...
    $config->software({Name => (File::Spec->splitpath($0))[2], Version => '@CASAVA_VERSION_FULL@'});
    ...
    my $inputMakefile = File::Spec->catfile( '@CASAVA_FULL_DATADIR@', "makefiles", 'Makefile');
...
sub selfTest
{
    ...
    if ( $exitValue != 0 )
    {
        ...
        my $colorizeSh = File::Spec->catfile( '@CASAVA_FULL_LIBEXECDIR@', 'colorize.sh');
