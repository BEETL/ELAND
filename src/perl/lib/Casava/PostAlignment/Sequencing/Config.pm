...
sub inputQC ($$\%\%\%\%$) {
    ...
    my (
      ...
      = @_;
    my $sortExportCmd = File::Spec->catfile('@CASAVA_FULL_LIBEXECDIR@', $CONF_APP_Ref->{cmdExport});
    my $maxNumberOfBins;
    my $error = 0;
    if ( !-d $projectDir ) {
        errorExit
...
sub configureBuild(\@$)
{
    my ($refAlignSampleDirs, $buildDir) = @_;

    my $reportsCmd  = File::Spec->catfile( '@CASAVA_FULL_LIBEXECDIR@', $CONF_APP{cmdReports} );
    my $runConfPath = File::Spec->catfile( $buildDir, $CONF_APP{dirConf}, $CONF_APP{runConfFileName} );
    
    if (!$CONF_PROJ{inSortedBam})
    {
    ...
    {
        $CONF_PROJ{inSortedBam} = abs_path($CONF_PROJ{inSortedBam});
        ...
        createDirs($buildDir, @dirs);
        my $genomeBamFilePath = File::Spec->catdir($buildDir, $genomeDirName, $bamDirName, 'sorted.bam');
         
        $CONF_PROJ{alignSortedBam} = abs_path($genomeBamFilePath); 
    }
    

    inputQC( $buildDir, $CONF_PROJ{dirBuildParsed}, %chrEnds, %CONF_PROJ, %runsConfig, %CONF_APP, !$CONF_PROJ{alignSortedBam});
...
sub configureTargets($\@\@$) {
    ...
    if ( $confStatus == 0 ) {
        ...
        configureBuild(@$refAlignSampleDirs, $buildDir) if grep (/^sort$|^all$/, @targets);
        @{$targetRef} = ();
...
sub scanDirRefSeq($\%\%) {
    ...
    my $countFastaBasesCmd = File::Spec->catfile('@CASAVA_FULL_LIBEXECDIR@', 'countFastaBases') . " $filesString";
...
sub loadConfiguration($;\%) {
    ...
    my $globalConf  = File::Spec->catfile( '@CASAVA_FULL_ETCDIR@', $globalAppConfFileName );
    ...
    my $currentAppMajorMinor = '@CASAVA_VERSION_FULL@';
    ...
    if (defined $CONF_PROJ{dirBuildTemp})
    {
        ...
        unless ( -d $CONF_PROJ{dirBuildTemp} ) {
            mkdir $CONF_PROJ{dirBuildTemp}, 0775
              or errorExit
                "ERROR: createDirs() can't make $CONF_PROJ{dirBuildTemp} $!\n";
        }
    }
...
sub updateRunConf(\%;$;\%;\%) {
    ...
    ${$confNewRunRef}{appVersion} = '@CASAVA_VERSION_FULL@';
