...
my $DATA_DIR = '@CASAVA_FULL_DATADIR@';
...
{
	my $self = shift;
    if (@_) {$self->{REDUCED_QSCORE} = shift};
    return $self->{REDUCED_QSCORE};
}
...
sub saveDemultiplexConfig($$$)
{
    ...
    $dc->software({Name => (File::Spec->splitpath($0))[2], Version => '@CASAVA_VERSION_FULL@'});
...
EOF
    ...
    errorExit "ERROR: Unable to detect read cycles from config.xml when the following mask applied: $self->{MASK}" unless defined $readCycles;
...
#    print $handle "QSEQ_MASK:=".$self->mask()."\n";
    print $handle "DEMUX_OPTIONS:=" . 
                  ...
                  ($self->reducedQscore() ? " --reduced-qscore" : "").
                  ...
                  ($self->fastqClusterCount() ? " --fastq-cluster-count=" . $self->fastqClusterCount() : "" ).
